...
const Session = (props) => (
    <SessionStyle.SessionDiv data-test="movie-day">
        ...
        <SessionStyle.SessionTimeDiv>
            {props.showtimes.map(showtime => <SessionStyle.SessionTimeLink key={showtime.id} to={"/assentos/" + showtime.id} data-test="showtime">{showtime.name}</SessionStyle.SessionTimeLink>)}
        </SessionStyle.SessionTimeDiv>
